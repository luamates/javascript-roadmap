  ...
  if (pathname === '/api/pedidos') {
    if (req.method === 'GET') {
      fs.readFile(DB_FILE, 'utf8', (err, data) => {
        if (err) {
          return sendJSON(res, 500, { error: 'Erro lendo o arquivo de pedidos' });
        }
        try {
      ...
      req.on('end', () => {
        try {
          ...
          fs.readFile(DB_FILE, 'utf8', (err, data) => {
            ...
            arr.push(pedido);
            fs.writeFile(DB_FILE, JSON.stringify(arr, null, 2), 'utf8', (err) => {
              if (err) return sendJSON(res, 500, { error: 'Erro salvando DB' });
